...
export abstract class Boggle implements BoggleGameModeInterface {
	...
	/**
	 * Verify that the word is a valid word in the dictionary
	 * @param word
	 */
	verify = (word: string): boolean => {
		if (!this.wordlist) {
	...
	/**
	 * Find out if the word can be created from the index
	 * @param word The word to serach for
	 * @param boggle The Boggle board
	...
	private findWordFromIndex = (
		...
		for (let row = y - 1 >= 0 ? y - 1 : 0; row < size && row <= y + 1; row++) {
			for (
	...
	/**
	 * Make sure the player is allowed to add this word to his/her wordlist.
	 * Mostly used for Battle game modes.
	 * @param word
	 */
	add = (word: string): boolean => {

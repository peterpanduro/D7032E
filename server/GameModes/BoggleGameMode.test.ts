...
Deno.test({
	...
	fn: () => {
		...
		assert(gameMode.canPlay("1+2=3", boggle));
	},
});

Deno.test({
	name: "Can't play expression",
	fn: () => {
		const gameMode = new Foggle16();
		const boggle = [
			["1", "0", "0", "0"],
...
Deno.test({
	name: "Can play expression from non-index [0, 0]",
	fn: () => {
		const gameMode = new Foggle16();
		const boggle = [
			["0", "0", "0", "0"],
		...
		assert(gameMode.canPlay("1*2+3=5", boggle));
	},
});

Deno.test({
	name: "Can play longer expression",
	fn: () => {
		const gameMode = new Foggle16();
		const boggle = [
			["0", "0", "0", "0"],
		...
		assert(gameMode.canPlay("1+2*3=5+2", boggle));
	},
});
